...
InvoiceFlow AI is an AI-assisted finance workflow for policy-grounded invoice
review, AR follow-up drafting, human review, audit trails, and deterministic
evaluations.
...
- Built a FastAPI-based finance workflow that extracts invoice and finance-email data, routes cases into AP or AR flows, and returns policy-grounded approval recommendations or follow-up drafts.
- Added optional schema-shaped LLM extraction, OCR fallback, schema validation, anomaly detection, lexical policy retrieval with repair, a tool-like workflow trace, and confidence-based human review.
...
- Built a FastAPI-based finance operations workflow that achieved `100%` pass rate, workflow-routing accuracy, extraction-field match, citation coverage, and grounding support on the bundled seven-case synthetic evaluation set.
- Added lexical retrieval over `25` citeable finance-policy chunks plus retrieval repair, schema validation, OCR fallback, a tool-like workflow trace, and anomaly detection for AP and AR workflows.
- Shipped a polished operator UI and CI eval gate so recommendations, evidence, retrieval repair, optional LLM gateway metadata, and latency can be inspected end to end.

### Short Version

- Built an AI-assisted FastAPI finance workflow with structured extraction, lexical policy retrieval, optional LLM extraction, AP/AR routing, human review, and grounded business-action outputs.
...
I built this project to simulate a finance operations console, not a generic chatbot. It takes invoice or finance-email input, produces strict structured output, retrieves and repairs policy evidence, routes the case into AP or AR logic, records inspectable workflow steps, applies a human-review gate, and returns a grounded business action through a FastAPI backend.

## Interview Talking Points

- The routing layer is deterministic so AP vs AR is resolved before downstream reasoning.
- The default retrieval layer uses lexical token overlap over small citeable policy chunks, then repairs missing required citations when first-pass retrieval is weak; it is not embedding or vector search.
- The optional LLM gateway centralizes schema-mode extraction calls, fallback behavior, PII-aware redaction, latency, and token metadata.
...
- AP/AR decision generation and the deployed default extraction path are deterministic. Optional LLM mode covers extraction and repair, while policy retrieval remains lexical rather than embedding/vector based.
